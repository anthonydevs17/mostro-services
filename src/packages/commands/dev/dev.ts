...
const startServer = async (
  ...
  try {
    ...
    const instrumentation = import.meta.resolve(
    ...
    if (
      currentServerProcess?.exitCode &&
      currentServerProcess?.exitCode !== 0
    ) {
      if (!currentServerProcess) {
        throw new Error(`Server failed to start`);
      }
      throw new Error(
        `Server failed to start with error: ${currentServerProcess.stderr}`
      );
    }
    ...
    if (currentServerProcess.exitCode !== null) {
      logger.error("Server failed to start with error:", {
        message: currentServerProcess.stderr
      });
      return;
    }
  } catch (err) {
    ...
    if (execaError.stdout)
      logger.debug("Server output:", { stdout: execaError.stdout });
  }
};
...
// Add this at the end of the file to run the dev function when the file is executed directly
if (import.meta.url === `file://${process.argv[1]}`) {
  dev({ port: 4111 }).catch(err => {
